 ...
 * -----------------------------------------------------------------------------------
 * | entity_id | creator_id | key (fn_name) | scopes | created_at | updated_at | ... |
 * -----------------------------------------------------------------------------------
 * entity_id - (PK)
 ...
 * scopes - comma separated scopes [{action}:{access_type}] - ex: ec2:full_read
 *
 * @param event
 * @param context
 */
export async function handler(
  ...
  context: Context
): Promise<GeneratePolicyDocument> {
   ...
   *
   * We could then query the role & scopes required for the functions using the methodArn (check scopes table above)
   * and validate that against the user's or we could offload authorization to the lambda function
   *
   */
  return generatePolicyDocument({
    principalId: userAccessTokenPayload.sub,
    methodArn,
    effect: 'Allow'
  });
}
